...
function allMyMaps() {
    ...
    getServerData("ws/impl/mapimpl/getmap", result => {
        ...
        for (var j = 0; j < Object.keys(result).length; j++) {
            ...
            //'<a class="dropdown-item" type="button" onclick=\"(changeMyCurrentMap(\''+tabName[j]+'\',\''+tabLat[j]+'\',\''+tabLon[j]+'\','+tabMarkerList[j]+'))"\>'+tabName[j]+'</a>'
            //'<a class="dropdown-item" type="button" onclick="tester('+tabMarkerList[j]+');">'+tabName[j]+'</a>'
        }
    });
}
//======================================================================================================================
function changeMyCurrentMap_(a, b, c, d){
    changeMyCurrentMap(a, b, c, JSON.parse(d));
}
function tester(tab){
    console.log(tab);
}
