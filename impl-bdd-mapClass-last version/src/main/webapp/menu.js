window.onload = function() {    //Toutes les fonctions ci-dessous s'execute à chaque refresh
    ...
    var map = changeMyCurrentMap(48.852969, 2.349903, /*map*/);

    allMyMaps(map);

    //Suppression de map
    $(".my-delete-map-button").click(function(){ //Bouton pour appeler la modal de création de nouvelles map
        getServerData("ws/impl/mapimpl/getmap", result => {
            $('#modalMapDeleter').modal('show');
            ...
            /*for (var i = 0; i < Object.keys(result).length; i++) {
                $("#navbar-item-1").append('<a class="dropdown-item" type="button" onclick="changeMyCurrentMap('+tabLat[i]+',' +tabLon[i]+')">' + tabName[i] + '</a>');
            }*/
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    document.getElementById("finisherMap").onclick = function(){ //Quand on a finit de créer la map, on l'envoie vers le serveur
        ...
        var physicalAddress = document.getElementById("nameCityRef").value;
...
function changeMyCurrentMap(lat, lon){
    $("#mainMap").remove();
...
function allMyMaps(currentMap) {
    getServerData("ws/impl/mapimpl/getmap", result => {
        var tabName = new Array();
        var tabLat = new Array();
        var tabLon = new Array();
        for (var i = 0; i < Object.keys(result).length; i++) {
            tabName[i] = result[i].name;     //récupération de tous les noms de map, on suppose que c'est celle du user
            tabLat[i] = result[i].latitudeDeparture;
            tabLon[i] = result[i].longitudeDeparture;
        }
        //Pour chaque nom, on crée un bouton dans la droplist
        for (var i = 0; i < Object.keys(result).length; i++) {
            $("#navbar-item-1").append('<a class="dropdown-item" type="button" onclick="changeMyCurrentMap('+tabLat[i]+',' +tabLon[i]+')">' + tabName[i] + '</a>');
